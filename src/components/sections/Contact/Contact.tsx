...
import { CheckCircle2, Github, Loader2, Mail, MapPin, Phone, Send, XCircle } from 'lucide-react';
...
const contactDetails = [
  { icon: Mail, label: 'Email', value: 'sithu669315@gmail.com', href: 'mailto:sithu669315@gmail.com' },
  { icon: Phone, label: 'Phone', value: '09-698412557', href: 'tel:09-698412557' },
  { icon: MapPin, label: 'Location', value: 'Meiktila,Mandalay', href: undefined },
  { icon: Github, label: 'GitHub', value: '/sithu669315-hash', href: 'https://github.com/sithu669315-hash' },
];
  ...
  const onSubmit = async (values: ContactFormValues) => {
    ...
    try {
      ...
      if (!serviceId || !templateId || !publicKey) {
        throw new Error('EmailJS is not configured. Add your keys to .env — see .env.example.');
      }
          ...
          <GlassCard className="p-6 sm:p-8 lg:col-span-3" hover={false}>
            <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-5">
              <div className="grid gap-5 sm:grid-cols-2">
                <div>
                  <label htmlFor="name" className="mb-1.5 block text-xs font-medium text-light-muted dark:text-dark-muted">
                    Name
                  </label>
                  <input
                    id="name"
                ...
                <div>
                  <label htmlFor="email" className="mb-1.5 block text-xs font-medium text-light-muted dark:text-dark-muted">
                    Email
                  </label>
                  <input
                    id="email"
              ...
              <div>
                <label htmlFor="subject" className="mb-1.5 block text-xs font-medium text-light-muted dark:text-dark-muted">
                  Subject
                </label>
                <input
                  id="subject"
              ...
              <div>
                <label htmlFor="message" className="mb-1.5 block text-xs font-medium text-light-muted dark:text-dark-muted">
                  Message
                </label>
                <textarea
                  id="message"
                  rows={5}
                  {...register('message', { required: 'Add a message', minLength: { value: 10, message: 'A little more detail helps' } })}
              ...
              <Button
                ...
                icon={status === 'loading' ? <Loader2 size={16} className="animate-spin" /> : <Send size={16} />}
